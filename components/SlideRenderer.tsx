...
import { Edit3, Wand2, Loader2 } from 'lucide-react';
...
const ImageGeneratorOverlay = ({ 
  ...
  <button
    ...
    className="absolute top-4 right-4 bg-black/60 hover:bg-banana-500 hover:text-black text-white p-2 rounded-full backdrop-blur-sm transition-all shadow-lg border border-white/10 group z-20"
...
export const SlideRenderer: React.FC<SlideRendererProps> = ({ slide, isActive, onUpdate, onGenerateImage }) => {
  ...
  const handleMagicClick = () => {
    if (onGenerateImage) {
      onGenerateImage(slide.id, slide.visualDescription);
    }
  };
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        {/* Global Render Logic for Content Layers */}
        {slide.layout === SlideLayout.VisualFocus ? (
           // Special Case for Visual Focus: Full Canvas Image
           <div className="absolute inset-0 w-full h-full group">
             <div className="absolute inset-0">
                <ImageGeneratorOverlay isGenerating={slide.isImageGenerating} onGenerate={handleMagicClick} />
                <img src={slide.imageUrl} alt="Background" className={`w-full h-full object-cover transition-opacity duration-700 ${slide.isImageGenerating ? 'opacity-50 scale-105 blur-sm' : 'opacity-100 scale-100'}`} />
             </div>
             {/* Cinematic Overlay */}
             ...
             <div className="relative z-10 h-full w-full flex flex-col justify-center px-16 max-w-4xl">
                <span className="text-banana-500 font-mono text-sm tracking-[0.2em] uppercase mb-6 opacity-0 animate-[fadeIn_0.5s_ease-out_forwards]">Visual Focus</span>
                <EditableArea
                  value={slide.title}
                  onChange={(v) => onUpdate(slide.id, 'title', v)}
                  className="text-6xl font-black text-white leading-tight mb-8 drop-shadow-xl"
                />
                <div className="space-y-4 mb-8">
                  {slide.points?.map((pt, idx) => (
                    <motion.p 
                      ...
                      className="text-2xl text-gray-100 font-light border-l-4 border-banana-500 pl-4 bg-black/30 backdrop-blur-sm py-2 pr-4 rounded-r-lg w-fit"
                    >
                      {pt}
                    </motion.p>
                  ))}
                </div>
                <div className="opacity-0 group-hover:opacity-60 transition-opacity bg-black/60 p-2 rounded text-xs text-gray-400 max-w-lg border border-white/10">
              ...
              <div className="flex h-full gap-8">
                ...
                <div className="w-1/3 flex items-center justify-center">
                   <div className="relative w-full aspect-square bg-dark-800 rounded-2xl overflow-hidden border border-white/10 group">
                      <ImageGeneratorOverlay isGenerating={slide.isImageGenerating} onGenerate={handleMagicClick} />
                      <img src={slide.imageUrl} alt="Visual" className={`w-full h-full object-cover transition-opacity ${slide.isImageGenerating ? 'opacity-30' : 'opacity-60 group-hover:opacity-80'}`} />
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                         {!slide.isImageGenerating && <p className="text-xs text-center text-gray-500 px-4 bg-black/50 py-2 rounded">{slide.visualDescription}</p>}
                      </div>
                   </div>
                </div>
              </div>
             ...
             {slide.layout === SlideLayout.SplitImage && (
               <div className="flex h-full w-full absolute inset-0">
                 ...
                 <div className="w-1/2 h-full relative group">
                    <ImageGeneratorOverlay isGenerating={slide.isImageGenerating} onGenerate={handleMagicClick} />
                    <img src={slide.imageUrl} className={`w-full h-full object-cover transition-opacity ${slide.isImageGenerating ? 'opacity-30' : ''}`} />
